...
uint8 wenduLimit=40;//温度阀值
uint8 shiduLimit=30;//湿度阀值
uint8 lightLimit=30;//光照阀值

uint8 check_shidu=0;//检测到的湿度
uint8 check_light=0;//检测到的光照
uint8 check_wendu=0;//检测到的温度
...
static uint16 EndDeviceID = 0x0001; //终端ID，非常重要
...
{ 
  ...
  MT_UartRegisterTaskID(task_id); //注册串口任务
    ...
    P0SEL &= ~0x30;                  //设置P0.4,p05口为普通IO
    P0DIR |= 0x30;                  //设置p04,P0.5为输出

    P1SEL &= ~0x01;                  //设置P1.0口为普通IO
...
void readUserNv()
{
    ...
    if (NV_ITEM_UNINIT == re)
    {
        ...
        buff[0]=wenduLimit;//温度阀值
        buff[1]=shiduLimit;//湿度阀值
        buff[2]=lightLimit;//光照阀值
    ...
    }else if(SUCCESS == re)
    {
        ...
        {
            wenduLimit=buff[0];//温度阀值
            shiduLimit=buff[1];//湿度阀值
            lightLimit=buff[2];//光照阀值
        }
    }
...
void displayAndCtrl()
{
    ...
    //温湿度显示
    sprintf(buff, "温度:%02d 湿度:%02d", check_wendu,check_shidu);
    ...
    //光照显示
    sprintf(buff, "光照:%02d", check_light);
...
void SampleApp_MessageMSGCB( afIncomingMSGPacket_t *pkt )
{
    ...
    case SAMPLEAPP_END1:
        {
            wenduLimit=pkt->cmd.Data[0];//温度阀值
            shiduLimit=pkt->cmd.Data[1];//湿度阀值
            lightLimit=pkt->cmd.Data[2];//光照阀值
...
static void SerialApp_SendSensorsData()
{
    ...
    DHT11();                //获取温湿度
    adc= HalAdcRead(HAL_ADC_CHANNEL_6, HAL_ADC_RESOLUTION_14); //ADC 采样值 P06口
    ...
    //通过阀值控制电机
    {
        check_wendu=wendu;//检测到的温度
        check_shidu=shidu;//检测到的湿度
        check_light=temp;//检测到的光照
        displayAndCtrl();
    }

    SendBuf[4]=wenduLimit;//温度阀值
    SendBuf[5]=shiduLimit;//湿度阀值
    SendBuf[6]=lightLimit;//光照阀值
